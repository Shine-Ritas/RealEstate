<div class="bg-white dark:bg-surface-variant rounded-xl p-6 md:p-8 border border-outline">
    <h2 class="text-2xl font-bold text-on-surface mb-6">{{ __('guest.schedule_viewing_title') }}</h2>
    ...
    <form wire:submit.prevent="scheduleViewing" class="space-y-4">
        <div>
            <label for="name" class="block text-sm font-medium text-on-surface mb-2">{{ __('guest.form_your_name') }}</label>
            <input 
                type="text" 
                id="name"
                wire:model="name"
                ...
                placeholder="{{ __('guest.form_name_placeholder') }}"
            >
            @error('name') <span class="text-danger text-sm">{{ $message }}</span> @enderror
        </div>

        <div>
            <label for="email" class="block text-sm font-medium text-on-surface mb-2">{{ __('guest.form_email_address') }}</label>
            <input 
                type="email" 
                id="email"
                wire:model="email"
                class="w-full px-4 py-3 border border-outline rounded-lg bg-surface text-on-surface focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                placeholder="{{ __('guest.form_email_placeholder') }}"
            >
            @error('email') <span class="text-danger text-sm">{{ $message }}</span> @enderror
        </div>

        <div>
            <label for="phone" class="block text-sm font-medium text-on-surface mb-2">{{ __('guest.form_phone_number') }}</label>
            <input 
                type="tel" 
                id="phone"
                wire:model="phone"
                class="w-full px-4 py-3 border border-outline rounded-lg bg-surface text-on-surface focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                placeholder="{{ __('guest.form_phone_placeholder') }}"
            >
            @error('phone') <span class="text-danger text-sm">{{ $message }}</span> @enderror
        </div>

        <div>
            <label for="message" class="block text-sm font-medium text-on-surface mb-2">{{ __('guest.form_message') }}</label>
            <textarea 
                id="message"
                wire:model="message"
                rows="4"
                ...
                placeholder="{{ __('guest.form_message_placeholder') }}"
            ></textarea>
